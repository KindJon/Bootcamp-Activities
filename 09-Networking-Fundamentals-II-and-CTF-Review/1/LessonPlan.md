...
- On Day 2, there is an optional review in the second half of class.  If any of today's or the next class activities and lessons need additional time, feel free to adjust them as needed with additional time since Day 2 has over an hour of optional reviews.
...
Students should be aware of denial of service (DoS) attacks. Explain that a DHCP starvation attack is a type of DoS attack that impacts the **availability** aspect of the CIA (confidentiality, integrity, and availability) triad.
...
Remind the class that we have covered many of the devices, protocols, and processes that move data from sources to destinations. We'll reference these devices as we cover routing schemes in the following section.
...
1. __Distance__: The amount of devices or hops used to get the data from the source to the destination.
  ...
  - Distance-vector routing protocols include:
    - **Routing Information Protocol (RIP)**: One of the oldest dynamic protocols that uses the hops count as its main criteria for choosing the route.
...
In this activity, students had to analyze the network diagrams of several Acme Corp offices and determine the shortest "Time-Wise" path between multiple servers in each office to identify the shortest routes.
...
Explain WiFi security by covering the following:
  ...
  - In 2006, an even more secure wireless protocol was created called **WPA2**.

  - **WPA3**, which is the latest and more secure standard, was introduced in 2018. However, adoption of WPA3 has been slower because it requires hardware that is capable of supporting it, and many older devices do not. Moreover, for WPA3 to work, both the WAP and the device connecting to it must support WPA3.  As of July 2020, WPA3 is required on any devices that are wi-fi certified. 
...
Explain that in order to view whether they are using WPA, and find out the WPA version, expand the following trees in the Packet Details frame:
...
Remind the class that in the previous sections, we covered multiple versions of wireless security protocols, including the weaker wireless security protocols, such as WEP.
